...
if (typeof document !== 'undefined') (() => {
  ...
  function renderVoice() {
    ...
    const current=el('voice-current');current.replaceChildren();current.append(node('span','МАРШРУТ','small-label'),node('div',v.route||'Состояние маршрута недоступно','mono'),node('span','АКТИВНЫЙ ENDPOINT','small-label'),node('div',endpoint?`${endpoint.device||endpoint.client} → ${endpoint.destination}:${endpoint.port} · ${endpoint.service}`:'Сейчас не найден','mono'));
    const metrics=el('voice-metrics');metrics.replaceChildren();for(const [label,value] of [['Контрольный UDP',decimal(v.latency_ms)+' мс'],['Jitter проб',decimal(v.jitter_ms)+' мс'],['Потери проб',decimal(v.loss_percent)+'%'],['Резерв Mieru',v.fallback_ready?'ГОТОВ':'НЕТ'],['Окно',`${v.window||0} проб`]]){const box=node('div');box.append(node('span',label),node('strong',value));metrics.append(box);}
