...
(()=>{
 ...
 async function loadVoice(){const v=await api('voice_status');el('voice-status').textContent=!v.available?'Маршрутизация звонков не установлена':v.active?(v.healthy&&v.upstream_probe?'Включена · HY2 и UDP-канал исправны':v.healthy?'Включена · ожидается очередная UDP-проба':'Аварийно отключается'):'Выключена · сетей Telegram: '+(v.telegram_nets||0)+' · голосовых сетей Discord: '+(v.discord_nets||0)+' · IP Discord в DNS-кэше: '+(v.discord_ips||0);el('voice-start').disabled=!v.available||!v.hy2||v.active||(v.telegram_nets||0)<6;el('voice-stop').disabled=!v.active;return v;}
 ...
 loadVoice().catch(e=>error(e.message));
})();
